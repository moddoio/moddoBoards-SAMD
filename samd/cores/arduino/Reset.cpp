...
/* noinline is required: this function erases flash while executing, so it must
 * stay in .ramfunc and never be inlined into a flash-resident caller (LTO
 * would otherwise inline the single call site in tickReset()).
 */
__attribute__ ((long_call, section (".ramfunc"), noinline))
void banzai() {
