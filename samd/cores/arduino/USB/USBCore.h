...
/* The bootloader's hardware endpoint descriptor table (bl_cdc_export ABI)
 * ends at EP4: exactly one pluggable module fits. A larger value would let
 * PluggableUSB::plug() hand out EP5+, whose transfers would DMA through
 * memory past the shared table. */
