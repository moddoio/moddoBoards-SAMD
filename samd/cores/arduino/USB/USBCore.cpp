...
/* CDC_BOOTLOADER builds use the bootloader's USB driver instead of this
 * stack; BootloaderUSBComposite.cpp provides the minimal USBDeviceClass
 * surface the HID library needs. Compiling this file too would duplicate
 * USBDevice and its methods. */
#if (!SAMC) && !defined(CDC_BOOTLOADER)
...
uint8_t udd_ep_out_cache_buffer[5][64];

static __attribute__((__aligned__(4))) //__attribute__((__section__(".bss_hram0")))
uint8_t udd_ep_in_cache_buffer[5][64];
...
void USBDeviceClass::initEndpoints() {
	for (uint8_t i = 1; i < (sizeof(EndPoints) / sizeof(EndPoints[0])) && EndPoints[i] != 0; i++) {
	...
	else if (config == (USB_ENDPOINT_TYPE_BULK | USB_ENDPOINT_OUT(0)))
	{
		if (epHandlers[ep] == NULL) {
			...
			if (ep == CDC_ENDPOINT_OUT) {
				// Static instance keeps the heap out of the CDC path; the
				// function-local static is constructed on first arrival here,
				// the same moment the heap allocation used to happen.
				static DoubleBufferedEPOutHandler cdcEPOutHandler(usbd, CDC_ENDPOINT_OUT);
				epHandlers[ep] = &cdcEPOutHandler;
			}
			else
#endif
			{
#ifdef PLUGGABLE_USB_ENABLED
				// Bulk OUT endpoints added by pluggable modules still come
				// from the heap.
...
static char LastTransmitTimedOut[5] = {
	0,
	0,
	0,
	0,
