...
private:
	// Full-speed bulk endpoints are at most 64 bytes; the buffers are inline
	// members so the handler works without a heap.
	...
	volatile uint8_t data0[size] __attribute__((__aligned__(4)));
	uint32_t first0;
	volatile uint32_t last0;
	volatile bool ready0;

	volatile uint8_t data1[size] __attribute__((__aligned__(4)));
	uint32_t first1;
